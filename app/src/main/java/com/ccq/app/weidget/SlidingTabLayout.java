...
public class SlidingTabLayout extends HorizontalScrollView {
    ...
    public void setTabSelectBack(int slectResId, int nomResId) {
        selectedTabBackColor = slectResId;
        unselectedTabBackColor = nomResId;
    }
    ...
    //选中时候tab的背景资源ID
    private int selectedTabBackColor = -1;
    //未选中时候tab的背景资源ID
    private int unselectedTabBackColor = -1;
...
//        textView.setTypeface(Typeface.DEFAULT_BOLD);
    ...
    private void populateTabStrip() {
        ...
        for (int i = 0; i < frgCount; i++) {
            ...
            if (unselectedTabBackColor > 0){
                tabViewList.get(i).setBackgroundResource(unselectedTabBackColor);
            }
        }
        textViewList.get(mViewPager.getCurrentItem()).setTextColor(selectedTitleTextColor);
        if(selectedTabBackColor>0){
            tabViewList.get(mViewPager.getCurrentItem()).setBackgroundResource(selectedTabBackColor);
        }
    }
        ...
        public void onPageSelected(int position) {
            ...
            for (int i = 0; i < frgCount; i++) {
                if (position == i) {
                    ...
                    if(selectedTabBackColor>0){
                        tabViewList.get(i).setBackgroundResource(selectedTabBackColor);
                    }
                } else {
                    textViewList.get(i).setTextColor(unselectedTitleTextColor);
                    if(unselectedTabBackColor>0){
