...
/*
        ...
        c.drawText(mDatas.get(position).getSuspensionTag(), child.getPaddingLeft()+dp2px(child.getContext(),16), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        c.drawText(tag, child.getPaddingLeft()+dp2px(child.getContext(),16),
