...
public class JGApplication{
    ...
    public static int maxImgCount=9;               //允许选择图片最大数
    ...
    public static void setPicturePath(String appKey) {
        if (!jiguang.chat.utils.SharePreferenceManager.getCachedAppKey().equals(appKey)) {
            jiguang.chat.utils.SharePreferenceManager.setCachedAppKey(appKey);
            JGApplication.PICTURE_DIR = "sdcard/JChatDemo/pictures/" + appKey + "/";
        }
    }

    public static jiguang.chat.database.UserEntry getUserEntry() {
        return jiguang.chat.database.UserEntry.getUser(cn.jpush.im.android.api.JMessageClient.getMyInfo().getUserName(), cn.jpush.im.android.api.JMessageClient.getMyInfo().getAppKey());
    }
}
