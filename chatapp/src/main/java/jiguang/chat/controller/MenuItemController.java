...
public class MenuItemController implements View.OnClickListener {
    ...
    public void onClick(View v) {
        Intent intent;
        int i = v.getId();if(i==R.id.create_group_ll){mFragment.dismissPopWindow();
                intent = new Intent(mFragment.getContext(), CreateGroupActivity.class);
                mFragment.getContext().startActivity(intent);

}else if(i==R.id.add_friend_with_confirm_ll){mFragment.dismissPopWindow();
                intent = new Intent(mFragment.getContext(), SearchForAddFriendActivity.class);
                intent.setFlags(1);
                mFragment.startActivity(intent);

}else if(i==R.id.send_message_ll){mFragment.dismissPopWindow();
                intent = new Intent(mFragment.getContext(), SearchForAddFriendActivity.class);
                intent.setFlags(2);
                mFragment.startActivity(intent);

            //扫描二维码
}else if(i==R.id.ll_saoYiSao){intent = new Intent(mFragment.getContext(), CommonScanActivity.class);
