...
public class ConversationListController implements View.OnClickListener,
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        //点击会话条目
        int headerViewsCount = mConvListView.getHeaderViewsCount();
        Intent intent = new Intent();
        if (position >= headerViewsCount) {
            //这里-3是减掉添加的三个headView
            Conversation conv = mDatas.get(position - headerViewsCount);
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
        ...
        final Conversation conv = mDatas.get(position - headerViewsCount);
        if (conv != null) {
            View.OnClickListener listener = new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (i == R.id.jmui_top_conv_ll) {//已经置顶,去取消
                        ...
                        int headerViewsCount = mConvListView.getHeaderViewsCount();
                        if(position - headerViewsCount >= 0 ){
                            mDatas.remove(position - headerViewsCount);
                            if (mDatas.size() > 0) {
                                mConvListView.setNullConversation(true);
                            } else {
                                mConvListView.setNullConversation(false);
