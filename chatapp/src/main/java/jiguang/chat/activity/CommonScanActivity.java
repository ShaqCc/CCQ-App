...
public final class CommonScanActivity extends Activity implements ScanListener, View.OnClickListener {
    ...
    @BindView(jiguang.chat.R2.id.scan_image)
    ImageView scan_image;
    @BindView(jiguang.chat.R2.id.authorize_return)
    ImageView authorize_return;
    private int scanMode;//扫描模型（二维码）

    @BindView(jiguang.chat.R2.id.common_title_TV_center)
    TextView title;
    @BindView(jiguang.chat.R2.id.scan_hint)
    TextView scan_hint;
    @BindView(jiguang.chat.R2.id.tv_scan_result)
    ...
    public void onClick(View v) {
        int i = v.getId();if(i==R.id.iv_light){scanManager.switchLight();

}else if(i==R.id.qrcode_g_gallery){showPictures(PHOTOREQUESTCODE);

}else if(i==R.id.authorize_return){finish();
