            ...
            public boolean onLongClick(View v) {
                ...
                View.OnClickListener listener = new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        int i = v.getId();if(i==R.id.jmui_delete_conv_ll){savePicture(path, mDialog);

}else if(i==R.id.jmui_top_conv_ll){Intent intent = new Intent(BrowserViewPagerActivity.this, ForwardMsgActivity.class);
...
//        }
    ...
    private OnClickListener listener = new OnClickListener() {
        @Override
        public void onClick(View v) {
            int i1 = v.getId();if(i1==R.id.return_btn){int pathArray[] = new int[mPathList.size()];
                    for (int i = 0; i < pathArray.length; i++) {
                        pathArray[i] = 0;
                    }
...
}else if(i1==R.id.pick_picture_send_btn){mProgressDialog = new ProgressDialog(mContext);
                ...
                //点击显示原图按钮，下载原图
}else if(i1==R.id.load_image_btn){downloadOriginalPicture();
