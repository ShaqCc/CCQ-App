...
public class FriendSettingActivity extends BaseActivity implements SlipButton.OnChangedListener, View.OnClickListener {
    ...
    public void onClick(View v) {
        int i = v.getId();if(i==R.id.setNoteName){Intent intent = new Intent(FriendSettingActivity.this, SetNoteNameActivity.class);
                intent.putExtra("user", getIntent().getStringExtra("userName"));
                intent.putExtra("note", getIntent().getStringExtra("noteName"));
                startActivityForResult(intent, 1);

}else if(i==R.id.btn_deleteFriend){View.OnClickListener listener = new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        int i1 = v.getId();if(i1==R.id.jmui_cancel_btn){mDialog.dismiss();

}else if(i1==R.id.jmui_commit_btn){final Dialog dialog = DialogCreator.createLoadingDialog(FriendSettingActivity.this, getString(R.string.processing));
                                dialog.show();
            ...
            //发送好友名片
}else if(i==R.id.rl_business){Intent businessIntent = new Intent(FriendSettingActivity.this, ForwardMsgActivity.class);
    ...
    public void onChanged(int id, boolean checkState) {
        if(id==R.id.btn_addBlackList){final LoadDialog dialog = new LoadDialog(FriendSettingActivity.this, false, "正在设置");
                dialog.show();
